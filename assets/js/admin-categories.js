(function () {
  ...
  var application = document.querySelector('[data-categories-app]');
  if (!application) {
    return;
  }
  ...
  var searchStatus = document.getElementById('category-search-status');
  var resultsSummary = document.getElementById('category-results-summary');
  ...
  function createIconButton(type, category) {
    ...
    button.title = isEdit ? 'Modifier la catégorie' : 'Supprimer la catégorie';
    button.setAttribute('aria-label', (isEdit ? 'Modifier la catégorie ' : 'Supprimer la catégorie ') + category.nom);
    button.setAttribute(isEdit ? 'data-category-edit' : 'data-category-delete', '');
    button.setAttribute('data-category-id', category.id);
    button.setAttribute('data-category-name', category.nom);
    button.setAttribute('data-bs-toggle', 'modal');
    button.setAttribute('data-bs-target', isEdit ? '#edit-category-modal' : '#delete-category-modal');

    if (isEdit) {
      button.setAttribute('data-category-description', category.description || '');
      button.setAttribute('data-category-status', category.statut);
    }
  ...
  function renderRows(data) {
    ...
    if (!data.rows.length) {
      ...
      icon.setAttribute('aria-hidden', 'true');
      title.textContent = 'Aucune catégorie trouvée';
      text.textContent = 'Ajoutez une catégorie ou modifiez votre recherche.';
    ...
    data.rows.forEach(function (category, index) {
      ...
      status.className = 'admin-status-badge ' + (category.statut === 'Actif' ? 'is-active' : 'is-inactive');
      status.textContent = category.statut;
      actions.className = 'admin-actions-cell';
      actions.setAttribute('data-label', 'Actions');
      actions.appendChild(createIconButton('edit', category));
      actions.appendChild(createIconButton('delete', category));
      ...
      row.appendChild(createCell('Statut', status));
      row.appendChild(createCell('Date de création', category.date_creation_formatted));
  ...
  function categoryUrl(page, search) {
  ...
  function createPaginationLink(label, page, search, options) {
    ...
    if (!options.disabled) {
      element.href = categoryUrl(page, search);
  ...
  function updateSummary(data) {
    if (!data.total) {
      resultsSummary.textContent = 'Aucune catégorie';
      return;
    }

    resultsSummary.textContent = (data.offset + 1) + '–' + Math.min(data.offset + data.per_page, data.total) + ' sur ' + data.total;
  }
  ...
  function loadCategories(page) {
    ...
    fetch(url.toString(), {
      ...
      .then(function (payload) {
        ...
        searchStatus.textContent = payload.data.total + ' catégorie' + (payload.data.total === 1 ? '' : 's') + ' trouvée' + (payload.data.total === 1 ? '' : 's') + '.';
        window.history.replaceState({}, '', categoryUrl(payload.data.page, search));
      })
  ...
  if (pagination) {
    pagination.addEventListener('click', function (event) {
      ...
      event.preventDefault();
      loadCategories(parseInt(link.getAttribute('data-page'), 10) || 1);
    });
  }
  ...
  if (tableBody) {
    tableBody.addEventListener('click', function (event) {
      ...
      if (editButton) {
        document.getElementById('edit-category-id').value = editButton.getAttribute('data-category-id');
        document.getElementById('edit-category-name').value = editButton.getAttribute('data-category-name');
        document.getElementById('edit-category-description').value = editButton.getAttribute('data-category-description') || '';
        document.getElementById('edit-category-status').value = editButton.getAttribute('data-category-status');
      }

      if (deleteButton) {
        document.getElementById('delete-category-id').value = deleteButton.getAttribute('data-category-id');
        document.getElementById('delete-category-name').textContent = deleteButton.getAttribute('data-category-name');
      }
    });
  }
}());
