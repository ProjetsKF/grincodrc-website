...
$pageDescription = $product && $product['description'] !== ''
    ? grinco_catalogue_excerpt($product['description'], 160)
    : ($product ? 'Découvrez ' . $product['nom'] . ' dans le catalogue GRINCO RDC.' : 'Le produit demandé est introuvable.');
$currentPage = 'produit';
  ...
  <section class="product-detail-section section"><div class="container">
    ...
    <?php else: ?>
      ...
      <div class="row g-5 align-items-start">
        <div class="col-lg-6"><div class="product-detail-image"><img src="<?php echo htmlspecialchars($product['image_url'] !== '' ? $product['image_url'] : grinco_url('/assets/img/logo.webp'), ENT_QUOTES, 'UTF-8'); ?>" alt="<?php echo htmlspecialchars($product['nom'], ENT_QUOTES, 'UTF-8'); ?>"></div></div>
        <div class="col-lg-6"><div class="product-detail-copy"><span class="product-detail-reference"><?php echo htmlspecialchars($product['reference'], ENT_QUOTES, 'UTF-8'); ?></span><h2><?php echo htmlspecialchars($product['nom'], ENT_QUOTES, 'UTF-8'); ?></h2><dl><div><dt>Catégorie</dt><dd><?php echo htmlspecialchars($product['categorie_nom'], ENT_QUOTES, 'UTF-8'); ?></dd></div><div><dt>Marque</dt><dd><?php echo htmlspecialchars($product['marque_nom'], ENT_QUOTES, 'UTF-8'); ?></dd></div><?php if ($product['modele'] !== ''): ?><div><dt>Modèle</dt><dd><?php echo htmlspecialchars($product['modele'], ENT_QUOTES, 'UTF-8'); ?></dd></div><?php endif; ?></dl><?php if ($product['description'] !== ''): ?><p><?php echo nl2br(htmlspecialchars($product['description'], ENT_QUOTES, 'UTF-8')); ?></p><?php endif; ?><div class="product-detail-add"><a class="catalogue-add-button" href="<?php echo grinco_url_html('/demande-devis?produit=' . (int) $product['id']); ?>"><i class="bi bi-file-earmark-text" aria-hidden="true"></i> Demander un devis</a></div></div></div>
      </div>
